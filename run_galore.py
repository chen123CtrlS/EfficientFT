...
def main():
    ...
    if args.task_name is not None:
        try:
            raw_datasets = load_from_disk(
                    os.path.join(args.data_path, "glue", args.task_name)
                )
    ...
    if args.pad_to_max_length:
        ...
        data_collator = DataCollatorWithPadding(tokenizer, pad_to_multiple_of=(8 if accelerator.mixed_precision else None))
    ...
    if not args.enable_galore:
        ...
        for module_name, module in model.named_modules():
            ...
            if 'value' in module_name:
                galore_params_V.append(module.weight)
            else:
                galore_params_QK.append(module.weight)
        galore_params = galore_params_QK + galore_params_V
        ...
        param_groups = [
            # {'params': regular_params}, 
                        {'params': galore_params_V, "lr": args.lr * args.lr_ratio, 'rank': args.lora_r, 'update_proj_gap': args.update_proj_gap, 'scale': args.galore_scale, 'proj_type': args.proj_type},
                        {'params': galore_params_QK, "lr": args.lr, 'rank': args.lora_r, 'update_proj_gap': args.update_proj_gap, 'scale': args.galore_scale, 'proj_type': args.proj_type}]
        optimizer = GaLoreAdamW(param_groups, lr=args.learning_rate)
    
    print('finish load optimizer!\n')
    total_params = sum(p.numel() for group in optimizer.param_groups for p in group['params'] if p.requires_grad)
    print(f"The total number of trainable parameters in the optimizer: {total_params}")
